...
print("""
date = Date(timeIntervalSinceReferenceDate: \(date.timeIntervalSinceReferenceDate))
""")
        var smallerIndex = 0
        for smaller in units {
            ...
            for larger in units {
                ...
                print("""
XCTAssertEqual(calendar.ordinality(of: .\(smaller), in: .\(larger), for: date), \(expected != nil ? String(expected!) : "nil"))
""")
                largerIndex += 1
            }
            smallerIndex += 1
...
//            test(.distantFuture, icuCalendar: icuCalendar, gregorianCalendar: gregorianCalendar)
    ...
    func verifyAdding(_ components: DateComponents, to date: Date, icuCalendar: _CalendarICU, gregorianCalendar: _CalendarGregorian, wrap: Bool = false, _ message: @autoclosure () -> String = "", file: StaticString = #file, line: UInt = #line) {
        ...
        expectEqual(added_greg, added_icu, message().appending("components:\(components). old: \(added_icu.timeIntervalSince1970). new: \(added_greg.timeIntervalSince1970)"), file: file, line: line)
    }
    ...
    func testAddComponentsCompatibility_RemoteDates() {
        let firstWeekday = 1
        let minimumDaysInFirstWeek = 1
        let timeZone = TimeZone(identifier: "America/Los_Angeles")!
        let icuCalendar = _CalendarICU(identifier: .gregorian, timeZone: timeZone, locale: nil, firstWeekday: firstWeekday, minimumDaysInFirstWeek: minimumDaysInFirstWeek, gregorianStartDate: nil)
        ...
        var date = Date(timeIntervalSince1970: -210866774822) // Some remote dates

        verifyAdding(.init(second: -1), to: date, icuCalendar: icuCalendar, gregorianCalendar: gregorianCalendar, wrap: false)
    }
...
//                let r = gregorianCalendar.firstInstant(of: .day, at: date)
    ...
    func testOrdinality_DST2() {
        let calendar = _CalendarICU(identifier: .gregorian, timeZone: TimeZone(identifier: "America/Los_Angeles")!, locale: nil, firstWeekday: nil, minimumDaysInFirstWeek: nil, gregorianStartDate: nil)
        var date = Date(timeIntervalSinceReferenceDate: 682898558.712307)
        ...
        XCTAssertEqual(calendar.ordinality(of: .nanosecond, in: .yearForWeekOfYear, for: date), 20649600712306977)
